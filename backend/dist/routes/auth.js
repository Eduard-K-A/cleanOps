...
const signupSchema = zod_1.z.object({
    ...
    role: zod_1.z.enum(['customer', 'employee']).optional().default('customer'),
});
/**
 ...
 */
router.patch('/me', auth_1.verifyAuth, async (req, res) => {
    try {
        if (!req.user) {
        ...
        if (typeof req.body.full_name === 'string')
            allowed.full_name = req.body.full_name.trim();
        if (typeof req.body.location_lat === 'number')
            allowed.location_lat = req.body.location_lat;
        if (typeof req.body.location_lng === 'number')
            allowed.location_lng = req.body.location_lng;
        if (typeof req.body.onboarding_completed === 'boolean')
            allowed.onboarding_completed = req.body.onboarding_completed;
