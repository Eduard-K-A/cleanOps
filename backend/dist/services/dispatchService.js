...
exports.dispatchHighUrgencyJobs = dispatchHighUrgencyJobs;
 ...
 */
async function dispatchHighUrgencyJobs() {
    try {
        ...
        for (const job of jobs) {
            ...
            try {
                ...
                const { data: employees, error: employeesError } = await supabase.rpc('get_nearby_employees', {
                ...
                if (notifyError) {
                    ...
                    console.log(`[Dispatch Service] Notified ${notifications.length} employee(s) about job ${job.id}`);
                }
            }
            catch (error) {
